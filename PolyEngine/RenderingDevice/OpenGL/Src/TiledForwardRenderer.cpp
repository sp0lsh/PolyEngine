...
TiledForwardRenderer::TiledForwardRenderer(GLRenderingDevice* rdi)
	: IRendererInterface(rdi), EnvironmentCapture(rdi), LastViewportRect(Vector2f::ZERO, Vector2f::ONE),
...
void TiledForwardRenderer::RenderShadowMap(const SceneView& sceneView)
{
	...
	for (const MeshRenderingComponent* meshCmp : sceneView.DirShadowOpaqueList)
	{
...
void TiledForwardRenderer::ComputeLightCulling(const SceneView& sceneView)
{
	...
	LightCullingShader.SetUniform("uScreenSizeX", viewportWidth);
	LightCullingShader.SetUniform("uScreenSizeY", viewportHeight);
...
void TiledForwardRenderer::RenderTranslucentLit(const SceneView& sceneView)
{
	...
	TranslucentShader.SetUniform("uLightCount", (int)std::min((int)sceneView.PointLightList.GetSize(), MAX_NUM_LIGHTS));
...
void TiledForwardRenderer::RenderOrthoMeshes(const SceneView& sceneView)
{
	...
	for (const OrthoMeshRenderingComponent* meshCmp : sceneView.OrthoList)
	{	
...
void TiledForwardRenderer::PostGamma(const SceneView& sceneView)
{
	...
	GammaShader.SetUniform("uSplashTint", Color::WHITE * 0.8f);
